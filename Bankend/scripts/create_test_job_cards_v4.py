...
def create_test_job_cards_robust():
    ...
    type_defaults = {}
    cur.execute('PRAGMA table_info(job_cards_jobcard)')
    for row in cur.fetchall():
        col_name = row[1]
        col_type = row[2].upper()
        if any(t in col_type for t in ['INT', 'DECIMAL', 'REAL', 'BOOL']):
    ...
    for u in usernames:
        ...
        for i in range(1, 3):
            ...
            data.update({
                ...
                'estimation_number': f"EST-{job_num}",
                'inspection_number': f"INSP-{job_num}",
                'appointment_number': f"APP-{job_num}",
                'date': today_str,
                ...
                'created_at': now_str,
                'updated_at': now_str,
                'portal_token': str(uuid.uuid4()).replace('-', '')[:32], # char(32)
                'plate_emirate': 'Dubai',
                'order_type': 'Normal',
            ...
            # Remove 'id' if it's in data (sqlite handles it)
            if 'id' in data:
                del data['id']
