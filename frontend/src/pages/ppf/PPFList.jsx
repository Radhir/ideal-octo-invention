...
const PPFList = () => {
    ...
    const fetchWarranties = async () => {
        try {
            const res = await axios.get('/forms/ppf/api/warranties/');
    ...
    return (
        <div style={{ padding: '30px 20px' }}>
            <PrintHeader title="PPF Warranty Registry" />
            <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '30px' }}>
                <div>
                    <h1 style={{ fontFamily: 'Outfit, sans-serif', color: '#b08d57', fontSize: '1.8rem' }}>PPF Warranty Registrations</h1>
                    <p style={{ color: '#94a3b8' }}>Manage and track PPF service history</p>
                </div>
                <div style={{ display: 'flex', gap: '15px' }}>
                    ...
                    <Link to="/ppf/create" className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '8px', textDecoration: 'none' }}>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', color: '#f8fafc' }}>
                    <thead>
                        <tr style={{ borderBottom: '1px solid rgba(255,255,255,0.1)', background: 'rgba(255,255,255,0.02)' }}>
                            ...
                            <th style={{ padding: '15px', textAlign: 'left', fontSize: '12px', textTransform: 'uppercase', color: '#b08d57' }}>Plate</th>
                        ...
                        {loading ? (
                            ...
                            filteredWarranties.map((w) => (
                                <tr key={w.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                                    ...
                                    <td style={{ padding: '15px' }}><span style={{ background: 'rgba(255,255,255,0.1)', padding: '4px 8px', borderRadius: '4px' }}>{w.license_plate}</span></td>
                                    <td style={{ padding: '15px' }}>{new Date(w.installation_date).toLocaleDateString()}</td>
                                    <td style={{ padding: '15px', textAlign: 'right' }}>
                                        <Link to={`/ppf/${w.id}`} style={{ color: '#b08d57' }}><Eye size={20} /></Link>
                                    </td>
                                </tr>
                            ))
                        )}
